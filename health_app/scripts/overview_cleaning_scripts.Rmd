...
# health board names
```{r}
health_names <- read_csv(here("data/clean_data/health_board_names.csv"))

 greenspace_health_names <- health_names %>% 
  select(area_code, hb_name) %>% 
  distinct() %>% 
  right_join(greenspace_clean, by = "area_code")
 
 write_csv(greenspace_health_names, here("data/clean_data/greenspace_health_names.csv"))
```
...
scotland_health_survey_local <- scotland_health_survey_local %>% 
  ...
  scotland_health_survey_local <- health_board_names %>% 
    select(area_code, hb_name) %>% 
    distinct() %>% 
