...
summary_stat_scottish_health_survey_local_area <- raw_scotland_health_survey_local %>% 
  ...
  mutate(
    proportion = as.numeric(value)/100
  )
  
summary_stat_scotland_health_2016_2019 <- summary_stat_scottish_health_survey_local_area %>% 
  rename(year = date_code) %>% 
  select(scottish_health_survey_indicator, sex, measurement, proportion)
